...
export default function LatestPosts() {
  ...
  useEffect(() => {
    // Зареждаме последните 3 публикации от WordPress API
    async function fetchPosts() {
      const res = await fetch(
        "https://brd.devclick.net/wp-json/wp/v2/posts?per_page=3"
      ); // добавен е per_page=3
    ...
    <div className="bg-white">
      <div className="mx-auto w-full py-0 sm:px-6 sm:py-0 lg:px-0">
        <div className="relative isolate overflow-hidden bg-gray-900 px-6 py-24 text-center shadow-2xl sm:px-16">
          ...
          <div className="mx-auto mt-16 grid !max-w-[80%] flex grid-cols-1 gap-x-8 gap-y-20 lg:mx-auto lg:max-w-none lg:grid-cols-3">
            {posts.length > 0 ? (
              posts.map((post) => (
                <article
                  key={post.id}
